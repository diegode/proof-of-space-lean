...
`E_d(x,x) = (2-d) H(x) > -ε` once `H(x)` is small enough, so at this fixed level the
certified region is *empty* near `0` and `1` — below `x ≈ 1.9·10⁻⁹` nothing is certifiable
at all, whatever profile is used.  `ChungRelative.lean` removes the restriction by
replacing the fixed level with one proportional to `H(x)`, and covers the two corners the
chord argument here cannot reach.
-/
