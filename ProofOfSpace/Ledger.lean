...
theorem extension_attempt_gen (L : CS.Link) :
  ...
  push Not at hend2
  rcases hout with hE | ⟨ht2E, hfert2, hexp2⟩
  · -- the attempt broke: charge it inside its own window
    ...
    have hsub : ∑ d ∈ Finset.Ico (t1 + 1) (E + 1), B.spend d
      ...
      omega
    linarith
  · have hactive : ∀ d, b ≤ d → d ≤ t2 → f d ∈ Set.Icc S.αmin S.αmax := by
      intro d hbd hdt2
      refine ⟨?_, (henc d hbd).2⟩
      by_cases hdt1 : d < t1
      ...
      · have ht1d : t1 ≤ d := by omega
        have hfloor := hfloorE d ht1d (lt_of_le_of_lt hdt2 ht2E)
        exact T.αmin_lt_lam.le.trans hfloor
    obtain ⟨L', hL'depth, hL'count⟩ :=
      CS.extend L t2 (by omega) hend2 hfert2 hexp2 hactive
