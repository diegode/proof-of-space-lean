...
The definitions in `Challenge.lean` import only Mathlib. They describe the
port wiring, static pebbling position, depth-robustness assumptions, and the
conclusion directly; probability machinery is confined to the proof files.

## Public results

The single registered theorem is `pebbling_latency`, stated in the self-contained
[Challenge.lean](Challenge.lean) and proved in [Solution.lean](Solution.lean).
[comparator.json](comparator.json) selects this theorem alone. The other two
results below remain proved supporting results in `Solution.lean`.
