...
# Comparator constructs Landrun's options itself but does not add that delimiter.
landrun_binary=${PALOMAR_LANDRUN_BIN:?PALOMAR_LANDRUN_BIN must name the pinned Landrun binary}
landrun_options=()

# Refuse flags that would let a future Comparator revision weaken the sandbox.
while [ "$#" -gt 0 ]; do
  case "$1" in
