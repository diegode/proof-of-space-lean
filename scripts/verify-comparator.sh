...
# Exact reviewed tool revisions. The exporter source is the Lean 4.33 release tag;
# patch release 4.33.1 has no separate lean4export tag, so it is compiled below with
# this project's exact toolchain.
...
expected_project_toolchain=leanprover/lean4:v4.33.1
...
if [ "$project_toolchain" != "$expected_project_toolchain" ]; then
  ...
  echo "update the lean4export pin and this guard when changing Lean" >&2
  exit 1
fi
